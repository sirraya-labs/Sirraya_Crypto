...
use super::hashers::{base_2b, f_hash, h_hash, prf, t_l};
...
pub fn fors_sk_gen(sk_seed: &[u8], pk_seed: &[u8], adrs: &Adrs, idx: u32, n: usize) -> Vec<u8> {
    let mut sk_adrs = *adrs;
    sk_adrs.set_type_and_clear(FORS_PRF);
    sk_adrs.set_key_pair_address(adrs.get_key_pair_address());
    sk_adrs.set_tree_index(idx);
    prf(pk_seed, sk_seed, sk_adrs.as_bytes(), n)
}

/// Algorithm 15: fors_node(SK.seed, i, z, PK.seed, ADRS).
pub fn fors_node(sk_seed: &[u8], i: u32, z: u32, pk_seed: &[u8], adrs: &mut Adrs, n: usize) -> Vec<u8> {
    if z == 0 {
        let sk = fors_sk_gen(sk_seed, pk_seed, adrs, i, n);
        adrs.set_tree_height(0);
        adrs.set_tree_index(i);
        f_hash(pk_seed, adrs.as_bytes(), &sk, n)
    } else {
        let lnode = fors_node(sk_seed, 2 * i, z - 1, pk_seed, adrs, n);
        let rnode = fors_node(sk_seed, 2 * i + 1, z - 1, pk_seed, adrs, n);
...
pub fn fors_sign(
    ...
    for (i, &idx) in indices.iter().enumerate() {
        sig.extend_from_slice(&fors_sk_gen(
            sk_seed,
            pk_seed,
            adrs,
            (i as u32) * (1u32 << a) + idx,
        ...
        for j in 0..a {
            ...
            sig.extend_from_slice(&fors_node(sk_seed, node_idx, j as u32, pk_seed, adrs, n));
        }
    }
    sig
}
...
pub fn fors_pk_from_sig(
    ...
    for (i, &idx) in indices.iter().enumerate() {
        ...
        let mut node = f_hash(pk_seed, adrs.as_bytes(), sk, n);
    ...
    t_l(pk_seed, forspk_adrs.as_bytes(), &roots, n)
}
